...
public class MessageService implements IDao<Message>{
    ...
    public boolean create(Message o) {
        ...
        String req="insert into message (objet, sujet,datee,idE,idR ) values(?,?,?,?,?)";
    ...
    public List<Message> findAll() {
        ...
        try{
          ...
          ResultSet rs=ps.executeQuery();
          while(rs.next()){
              employes.add(new Message(rs.getInt("id"),rs.getString("objet"),rs.getString("sujet"),rs.getDate("datee"),es.findById(rs.getInt("idE")),es.findById(rs.getInt("idR"))));
          }
        }catch (SQLException ex){
